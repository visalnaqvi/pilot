      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-5 sm:p-7">
        <section className="grid gap-6 lg:grid-cols-2">
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
            <div><p className="text-xs font-black tracking-widest text-indigo-600">TIMETABLE DETAILS</p><h3 className="mt-1 text-lg font-black">Schedule and groups</h3></div>
            ...
            <fieldset className="mt-6">
              <div className="flex flex-wrap items-end justify-between gap-3"><div><legend className="text-sm font-black">Groups</legend><p className="mt-1 text-xs text-slate-500">Selecting a group includes all its current members.</p></div><span className="text-xs font-bold text-indigo-700">{props.input.selectedGroupIds.length} selected</span></div>
              <label className="relative mt-3 block"><span className="sr-only">Search groups</span><SearchIcon /><input value={groupSearch} onChange={event => setGroupSearch(event.target.value)} placeholder="Search groups…" className="w-full rounded-xl border border-slate-300 bg-white py-2.5 pl-10 pr-4 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100" /></label>
              <div className="mt-3 grid max-h-64 gap-2 overflow-y-auto pr-1 sm:grid-cols-2">
                ...
                {!filteredGroups.length && <EmptyChoice text={props.groups.length ? 'No groups match this search.' : 'No groups available.'} />}
              </div>
            </fieldset>
          </div>

          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-black tracking-widest text-indigo-600">STUDENT SELECTION</p><h3 className="mt-1 text-lg font-black">Individual students</h3><p className="mt-1 text-xs text-slate-500">Add students directly, with or without a selected group.</p></div><span className="rounded-full bg-indigo-100 px-3 py-1 text-xs font-black text-indigo-700">{props.assigneeCount} unique</span></div>
