  ...
  return <section className="calendar-page flex h-[calc(100dvh-9.5rem)] min-h-[32rem] flex-col">
    ...
    <section className="assignment-calendar mt-4 min-h-0 flex-1 rounded-2xl border border-slate-200 bg-white p-3 shadow-[0_14px_40px_rgb(15_23_42/0.08)] sm:p-5">
      <FullCalendar
        ...
        eventContent={info => <div className="min-w-0 px-1 py-0.5"><p className="truncate font-extrabold">{info.event.extendedProps.kind === 'task' ? 'Task · ' : ''}{info.event.title}</p><p className="truncate text-[10px] opacity-85">{info.event.extendedProps.detail}</p></div>}
        nowIndicator
        allDaySlot={false}
      ...
      />
      {!events.length && <p className="pointer-events-none absolute bottom-8 left-1/2 z-10 -translate-x-1/2 rounded-full border border-slate-200 bg-white/95 px-4 py-2 text-sm text-slate-500 shadow-sm">{eventFilter === 'assignments' ? 'No assignments are scheduled.' : eventFilter === 'tasks' ? 'No tasks with deadlines are scheduled.' : manager ? 'Create an assignment or scheduled task to see it here.' : 'No assignments or tasks are scheduled yet.'}</p>}
    </section>
