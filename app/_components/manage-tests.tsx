...
export function ManageTests() {
  ...
  const isTeacher = profile?.role === 'user' && teacherOrganisations.length > 0
  const allowed = isAdmin || profile?.role === 'organisation' || isTeacher

  useEffect(() => {
    if (!user || !allowed) return
    const controller = new AbortController()
    authenticatedFetch(user, `/api/tests?owned=1${showDeleted ? '&deleted=1' : ''}`, { cache: 'no-store', signal: controller.signal })
      .then(async response => {
        const payload = await response.json()
        if (!response.ok) throw new Error(payload.error || 'Unable to load tests.')
        setTests(sort((payload.items || []).filter((test: MockTest) => showDeleted ? test.deletedAt != null : test.deletedAt == null)))
      })
      .catch(reason => {
        if (!controller.signal.aborted) setMessage(`Could not load tests: ${reason instanceof Error ? reason.message : 'Unknown error'}`)
      })
      .finally(() => {
  ...
  async function updateDeleted(test: MockTest, deleted: boolean) {
    if (!user || (deleted && !confirm(`Soft-delete “${test.title}”?`))) return
    setProcessing(test.id)
    try {
      const response = await authenticatedFetch(user, '/api/tests', {
        method: 'PATCH',
        body: JSON.stringify({ id: test.id, deleted }),
      })
      if (!response.ok) throw new Error((await response.json()).error || 'Unable to update this test.')
      setTests(items => items.filter(item => item.id !== test.id))
    } catch (reason) {
      setMessage(reason instanceof Error ? reason.message : `Unable to ${deleted ? 'delete' : 'restore'} this test.`)
    } finally {
      setProcessing('')
    }
  }
