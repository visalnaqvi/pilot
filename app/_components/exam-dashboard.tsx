...
function ExamUserList({ attempts, tests, groups, assignments, dashboardType = 'exam' }: { attempts: Submission[]; tests: MockTest[]; groups: Group[]; assignments: TestAssignment[]; dashboardType?: 'exam' | 'test' | 'group' }) {
  ...
  const labels = sortedChartDateKeys(daily.keys())
  const chartOption = { tooltip: { trigger: 'axis' }, grid: { left: 38, right: 16, top: 28, bottom: 48 }, xAxis: { type: 'category', data: labels, axisLabel: { rotate: 35 } }, yAxis: { type: 'value', min: 0, max: 100 }, series: [{ name: 'Average score', type: 'line', smooth: true, data: labels.map((label) => { const values = daily.get(label) || []; return Math.round(values.reduce((sum, item) => sum + score(item), 0) / values.length) }), areaStyle: { color: `${brand.primaryColor}22` }, lineStyle: { color: brand.primaryColor, width: 3 }, itemStyle: { color: brand.primaryColor } }] }
  ...
  return <section>
    ...
    <div className="mt-3 space-y-3">{users.map((user, index) => {
      ...
      return <article key={user.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="grid gap-5 xl:grid-cols-[minmax(15rem,1fr)_minmax(18rem,1.4fr)_minmax(17rem,1.2fr)] xl:items-center xl:gap-0">
        ...
        <div className="grid grid-cols-3 justify-items-start gap-3 xl:px-5"><ScoreRing value={100} display={String(user.attempts.length)} label="Attempts" color="var(--color-indigo-500)" /><ScoreRing value={average} display={`${average}%`} label="Average" color="var(--color-violet-600)" /><ScoreRing value={bestScore} display={`${bestScore}%`} label="Best" color="#10b981" /></div>
      </div>
      </article>
    })}</div>
...
<Chart title="Average score by date" option={option('Average score', averageByDay, brand.primaryColor)} labels={labels} onDateSelect={date => { setSelectedDate(date); setSelectedUsersPage(1) }} />
