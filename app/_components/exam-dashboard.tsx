...
<p className="mt-3 max-w-2xl text-slate-600">Select an exam to explore its performance, learners and group goals.</p>
</div>
...
<Cell label="Groups" value={item.groups} />

<Cell label="Group users" value={item.groupUsers} />
...
function DashboardOverview({ exams, tests, groups, submissions }: { exams: Exam[]; tests: MockTest[]; groups: Group[]; submissions: Submission[] }) {
  ...
  return <section className="mt-8 space-y-6">
    <div className="grid overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-lg shadow-slate-200/60 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <Metric icon="students" label="Number of students" value={students} note="Students with attempts or group membership" />
      <Metric icon="groups" label="Number of groups" value={groups.length} note="Available institute groups" />
    ...
    <div className="grid gap-6 xl:grid-cols-2">
      <InsightCard icon="trophy" label="Highest performing group" title={highestGroup?.group.name || '—'} detail={highestGroup ? `${Math.round(highestGroup.average)}% average across ${highestGroup.attempts.length} attempt${highestGroup.attempts.length === 1 ? '' : 's'}` : 'Will appear after a group member completes a test.'} />
      <InsightCard icon="star" label="Highest scoring user" title={highestUser?.userName || highestUser?.userEmail || highestUser?.userId || '—'} subtitle={highestUser?.userEmail || highestUser?.userId} detail={highestUser ? `${highestUser.score}/${highestUser.totalMarks} marks (${Math.round(score(highestUser))}%) on ${highestUser.testTitle}` : 'Will appear after a student completes a test.'} />
    </div>
    <div className="grid gap-6 xl:grid-cols-2">
      <TrendChart title="Group average score by date" filterLabel="Group" value={groupId} onChange={setGroupId} options={groups.map(group => ({ id: group.id, label: group.name }))} attempts={groupAttempts} emptyText="A dated group submission is needed to show this trend." />
...
function AttemptsByDateChart({ exams, tests, groups, submissions }: { exams: Exam[]; tests: MockTest[]; groups: Group[]; submissions: Submission[] }) {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div><h2 className="font-black">Number of attempts by date</h2><p className="mt-1 text-sm text-slate-500">Across all exams by default. Combine filters to narrow the trend.</p></div><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><DashboardPicker label="Exam" value={examId} options={[{ id: '', label: 'All exams' }, ...exams.map(exam => ({ id: exam.id, label: exam.name }))]} onChange={setExamId} placeholder="Search exams" /><DashboardPicker label="Test" value={testId} options={[{ id: '', label: 'All tests' }, ...tests.map(test => ({ id: test.id, label: test.title, detail: test.category || undefined }))]} onChange={setTestId} placeholder="Search tests" /><DashboardPicker label="User" value={userId} options={[{ id: '', label: 'All users' }, ...users.map(([id, email]) => ({ id, label: email }))]} onChange={setUserId} placeholder="Search users" /><DashboardPicker label="Group" value={groupId} options={[{ id: '', label: 'All groups' }, ...groups.map(group => ({ id: group.id, label: group.name, detail: `${group.members.length} users` }))]} onChange={setGroupId} placeholder="Search groups" /></div>{labels.length ? <ReactECharts option={option} style={{ height: 300 }} notMerge lazyUpdate /> : <Empty text="No dated attempts match the selected filters." />}</section>
}
...
const tabs = [['scores', 'Score analysis'], ['details', 'Exam details'], ['users', 'User list'], ['groups', 'Group goals']] as const
...
function ExamUserList({ attempts, tests, groups, assignments, dashboardType = 'exam' }: { attempts: Submission[]; tests: MockTest[]; groups: Group[]; assignments: TestAssignment[]; dashboardType?: 'exam' | 'test' | 'group' }) {
  ...
  return <section className="space-y-6">
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><ModalStat label={`Users who took this ${dashboardType}`} value={rankedUsers.length} /><ModalStat label={dashboardType === 'test' ? 'Test attempts' : dashboardType === 'group' ? 'Group members' : 'Users assigned this exam'} value={dashboardType === 'test' ? attempts.length : assignedUsers.size} /><ModalStat label={dashboardType === 'test' || dashboardType === 'group' ? 'Average score' : 'Groups assigned this exam'} value={dashboardType === 'test' || dashboardType === 'group' ? (attempts.length ? `${Math.round(attempts.reduce((sum, attempt) => sum + score(attempt), 0) / attempts.length)}%` : '—') : groups.length} /><ModalStat label="Participation" value={assignedUsers.size ? `${participation}%` : '—'} /></div>
    <section className="rounded-xl border p-4"><h3 className="text-xl font-bold tracking-tight text-slate-900">User score trend</h3><p className="mt-1 text-sm text-slate-500">{dashboardType === 'test' ? 'Shows this test’s users by default. Select a user to refine the chart.' : `Shows all users in this ${dashboardType} by default. Select a user and test to refine the chart.`}</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><SearchPicker value={selectedUserId} options={[{ id: '', label: 'All users', detail: `${users.length} users` }, ...users.map((user) => ({ id: user.id, label: user.name, detail: `${user.email} · ${user.attempts.length} attempts` }))]} onChange={(option) => setSelectedUserId(option.id)} placeholder="Search and select a user" />{dashboardType !== 'test' && <SearchPicker value={selectedTestId} options={[{ id: '', label: `All tests in this ${dashboardType}` }, ...tests.map((test) => ({ id: test.id, label: test.title, detail: test.category || undefined }))]} onChange={(option) => setSelectedTestId(option.id)} placeholder="Search and select a test" />}</div>{labels.length ? <ReactECharts option={chartOption} style={{ height: 280 }} notMerge lazyUpdate /> : <Empty text="A dated submission is needed to show this trend." />}</section>
    <UserRanking title="Top 5 users" users={rankedUsers.slice(0, 5)} /><UserRanking title="Lowest 5 users" users={rankedUsers.slice().reverse().slice(0, 5)} />
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between"><div><h3 className="text-xl font-bold tracking-tight text-slate-900">All users by attempts and average score</h3><p className="mt-1 text-sm text-slate-500">Filter, sort, and browse learner performance.</p></div><p className="text-sm font-medium text-slate-500">{rows.length} user{rows.length === 1 ? '' : 's'}</p></div><div className="mt-5 grid gap-3 md:grid-cols-3"><SearchPicker value={selectedUserId} options={[{ id: '', label: 'All users' }, ...users.map((user) => ({ id: user.id, label: user.name, detail: user.email }))]} onChange={(option) => { setSelectedUserId(option.id); setPage(1) }} placeholder="Search user" />{dashboardType !== 'test' && <SearchPicker value={selectedTestId} options={[{ id: '', label: 'All tests' }, ...tests.map((test) => ({ id: test.id, label: test.title, detail: test.category || undefined }))]} onChange={(option) => { setSelectedTestId(option.id); setPage(1) }} placeholder="Search test" />}{dashboardType === 'exam' && <SearchPicker value={selectedGroupName} options={[{ id: '', label: 'All groups' }, ...groups.map((group) => ({ id: group.name, label: group.name, detail: `${group.members.length} users` }))]} onChange={(option) => { setSelectedGroupName(option.id); setPage(1) }} placeholder="Search group" />}</div><div className="mt-5 overflow-x-auto rounded-xl border border-slate-200"><table className="w-full min-w-[720px] text-left text-sm"><thead className="bg-slate-50 text-xs font-bold tracking-wide text-slate-500"><tr><th className="px-4 py-3">User</th><th className="px-4 py-3"><button type="button" onClick={() => toggleSort('attempts')} className="inline-flex items-center gap-1 hover:text-indigo-700">Attempts <span aria-hidden="true">{sort.field === 'attempts' ? (sort.direction === 'asc' ? '↑' : '↓') : '↕'}</span></button></th><th className="px-4 py-3"><button type="button" onClick={() => toggleSort('average')} className="inline-flex items-center gap-1 hover:text-indigo-700">Average score <span aria-hidden="true">{sort.field === 'average' ? (sort.direction === 'asc' ? '↑' : '↓') : '↕'}</span></button></th><th className="px-4 py-3">Groups</th></tr></thead><tbody>{pagedRows.map((user) => <tr key={user.id} className="border-t border-slate-100 transition-colors hover:bg-indigo-50/50"><td className="px-4 py-3.5"><UserIdentity name={user.name} email={user.email} /></td><td className="px-4 py-3.5 font-semibold text-slate-800">{user.attempts.length}</td><td className="px-4 py-3.5"><span className="rounded-full bg-indigo-50 px-2.5 py-1 font-bold text-indigo-700">{user.attempts.length ? `${Math.round(user.average)}%` : '—'}</span></td><td className="max-w-72 px-4 py-3.5 text-slate-600">{user.groups.join(', ') || '—'}</td></tr>)}</tbody></table>{!pagedRows.length && <p className="p-6 text-center text-sm text-slate-500">No users match these filters.</p>}</div>{rows.length > pageSize && <div className="mt-4 flex items-center justify-between gap-3"><p className="text-sm text-slate-500">Showing {(currentPage - 1) * pageSize + 1}–{Math.min(currentPage * pageSize, rows.length)} of {rows.length}</p><div className="flex gap-2"><button type="button" disabled={currentPage === 1} onClick={() => setPage(currentPage - 1)} className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-40">Previous</button><button type="button" disabled={currentPage === totalPages} onClick={() => setPage(currentPage + 1)} className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-40">Next</button></div></div>}</section>
  </section>
  const search = userQuery
  const setSearch = setUserQuery
  const visibleUsers = users
  return <section className="space-y-6">
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><ModalStat label="Users who took this exam" value={users.length} /><ModalStat label="Users assigned this exam" value={assignedUsers.size} /><ModalStat label="Groups assigned this exam" value={groups.length} /><ModalStat label="Participation" value={assignedUsers.size ? `${participation}%` : '—'} /></div>
    <section className="rounded-xl border p-4"><h3 className="font-bold">User score trend</h3><p className="mt-1 text-sm text-slate-500">Defaults to the user with the most attempts. Select a user and test to refine the chart.</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><input list="exam-user-options" value={userQuery} onChange={(event) => { const value = event.target.value; setUserQuery(value); const match = users.find((user) => user.email === value); if (match) setSelectedUserId(match.id) }} type="search" placeholder="Search and select a user" className="rounded-lg border border-slate-300 px-3 py-2" /><datalist id="exam-user-options">{users.map((user) => <option key={user.id} value={user.email} />)}</datalist><input list="exam-test-options" value={testQuery} onChange={(event) => { const value = event.target.value; setTestQuery(value); setSelectedTestId(tests.find((test) => test.title === value)?.id || '') }} type="search" placeholder="Search and select a test" className="rounded-lg border border-slate-300 px-3 py-2" /><datalist id="exam-test-options">{tests.map((test) => <option key={test.id} value={test.title} />)}</datalist></div>{activeUser && labels.length ? <ReactECharts option={chartOption} style={{ height: 280 }} notMerge lazyUpdate /> : <Empty text="A dated submission is needed to show this user’s trend." />}</section>
    <UserRanking title="Top 5 users" users={users.slice(0, 5)} /><UserRanking title="Lowest 5 users" users={users.slice().reverse().slice(0, 5)} />
    <section><h3 className="font-bold">All users by attempts and average score</h3><div className="mt-3 overflow-x-auto rounded-xl border"><table className="w-full text-left text-sm"><thead className="bg-slate-50 text-xs text-slate-500"><tr><th className="p-3">User</th><th className="p-3">Attempts</th><th className="p-3">Average score</th><th className="p-3">Groups</th></tr></thead><tbody>{users.map((user) => <tr key={user.id} className="border-t"><td className="p-3 font-semibold">{user.email}</td><td className="p-3">{user.attempts.length}</td><td className="p-3 font-bold text-indigo-700">{Math.round(user.average)}%</td><td className="p-3 text-slate-600">{user.groups.join(', ') || '—'}</td></tr>)}</tbody></table>{!users.length && <p className="p-4 text-sm text-slate-500">No learner attempts are available yet.</p>}</div></section>
  </section>
  return <section className="space-y-6"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><ModalStat label="Users who took this exam" value={users.length} /><ModalStat label="Users assigned this exam" value={assignedUsers.size} /><ModalStat label="Groups assigned this exam" value={groups.length} /><ModalStat label="Participation" value={assignedUsers.size ? `${participation}%` : '—'} /></div><UserRanking title="Top 5 users" users={users.slice(0, 5)} /><UserRanking title="Lowest 5 users" users={users.slice().reverse().slice(0, 5)} /><section><h3 className="font-bold">All users by attempts and average score</h3><div className="mt-3 overflow-x-auto rounded-xl border"><table className="w-full text-left text-sm"><thead className="bg-slate-50 text-xs text-slate-500"><tr><th className="p-3">User</th><th className="p-3">Attempts</th><th className="p-3">Average score</th><th className="p-3">Groups</th></tr></thead><tbody>{users.map((user) => <tr key={user.id} className="border-t"><td className="p-3 font-semibold">{user.email}</td><td className="p-3">{user.attempts.length}</td><td className="p-3 font-bold text-indigo-700">{Math.round(user.average)}%</td><td className="p-3 text-slate-600">{user.groups.join(', ') || '—'}</td></tr>)}</tbody></table>{!users.length && <p className="p-4 text-sm text-slate-500">No learner attempts are available yet.</p>}</div></section><section className="rounded-xl border p-4"><h3 className="font-bold">User score trend</h3><p className="mt-1 text-sm text-slate-500">Defaults to the user with the most attempts.</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><input value={search} onChange={(event) => setSearch(event.target.value)} type="search" placeholder="Search a user" className="rounded-lg border border-slate-300 px-3 py-2" /><select value={activeUser?.id || ''} onChange={(event) => setSelectedUserId(event.target.value)} className="rounded-lg border border-slate-300 bg-white px-3 py-2"><option value="">Select a user</option>{visibleUsers.map((user) => <option key={user.id} value={user.id}>{user.email}</option>)}</select></div>{activeUser && labels.length ? <ReactECharts option={chartOption} style={{ height: 280 }} notMerge lazyUpdate /> : <Empty text="A dated submission is needed to show this user’s trend." />}</section></section>
}
...
<Chart title={scopeLabel === 'user' ? `Tests taken${selectedTest ? ` for ${selectedTest.title}` : ''} by date` : `Users who took this ${selectedTest ? 'test' : scopeLabel} by date`} option={option(scopeLabel === 'user' ? 'Tests' : 'Users', countByDay, '#059669')} labels={labels} onDateSelect={date => { setSelectedDate(date); setSelectedUsersPage(1) }} />

</div> : <Empty text="Date charts will appear once submissions have saved timestamps." />
}{labels.length > 0 && <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-2"><h3 className="text-xl font-bold tracking-tight text-slate-900">Users on {selectedDate}</h3><span className="text-sm font-medium text-slate-500">{selectedUsers.length} user{selectedUsers.length === 1 ? '' : 's'}</span></div><div className="mt-4 space-y-3">{visibleSelectedUsers.items.map(([id, item]) => { const average = item.attempts.reduce((sum, attempt) => sum + score(attempt), 0) / item.attempts.length; return <SubmissionUserCard key={id} submission={item.attempts[0]} average={average} attemptCount={item.attempts.length} /> })}</div>{!selectedUsers.length && <p className="mt-3 text-sm text-slate-500">No users submitted on this date.</p>}<Pagination page={visibleSelectedUsers.page} totalItems={selectedUsers.length} onPageChange={setSelectedUsersPage} itemLabel="users" className="mt-4 rounded-xl border border-slate-200" /></section>}
...
<PerformanceUserCard label="Highest score user" data={highestUser} />

<PerformanceUserCard label="Lowest score user" data={lowestUser} />
...
export function TestDashboardModal({ test, close }: { test: MockTest; close: () => void }) {
  ...
  const tabs = [['scores', 'Score analysis'], ['details', 'Test details'], ['users', 'User list']] as const
...
export function GroupDashboardModal({ group, submissions, tests, edit, close }: { group: Group; submissions: Submission[]; tests: MockTest[]; edit: () => void; close: () => void }) {
  ...
  const tabs = [['scores', 'Score analysis'], ['details', 'Group details'], ['users', 'User list']] as const
  if (tab === 'scores') return <ScoreAnalysisModal stats={stats} attempts={attempts} tests={groupTests} tabs={tabs} setTab={setTab} close={close} dashboardLabel="GROUP DASHBOARD" dashboardName={group.name} action={{ label: 'Edit group', onClick: edit }} scopeLabel="group" />
  return <DashboardModalShell label="GROUP DASHBOARD" name={group.name} activeTab={tab} tabs={tabs} setTab={setTab} edit={edit} close={close}>
    {tab === 'details' ? <section><h3 className="text-xl font-bold">Group details</h3><dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{[['Group name', group.name], ['Members', String(group.members.length)], ['Exam goal', group.targetExamName || 'No exam goal'], ['Tests attempted', String(groupTests.length)], ['Total attempts', String(attempts.length)], ['Participation', group.members.length ? `${Math.round(stats.completion)}%` : '—']].map(([label, value]) => <div key={label} className="rounded-xl border border-slate-200 p-4"><dt className="text-xs font-bold uppercase tracking-wide text-slate-500">{label}</dt><dd className="mt-1 font-semibold text-slate-900">{value}</dd></div>)}</dl></section> : <ExamUserList attempts={attempts} tests={groupTests} groups={[group]} assignments={[]} dashboardType="group" />}
  </DashboardModalShell>
}
...
export function OrganisationUserDashboardModal({ account, submissions, tests, goals, close }: { account: { uid: string; name: string; email: string }; submissions: Submission[]; tests: MockTest[]; goals: { id: string; name: string; examId?: string; examName?: string }[]; close: () => void }) {
  ...
  if (tab === 'scores') return <ScoreAnalysisModal stats={stats} attempts={attempts} tests={userTests} tabs={tabs} setTab={setTab} close={close} dashboardLabel="USER DASHBOARD" dashboardName={account.name} scopeLabel="user" />
  return <DashboardModalShell label="USER DASHBOARD" name={account.name} activeTab={tab} tabs={tabs} setTab={setTab} close={close}><ExamGoalProgress goals={goals} attempts={attempts} tests={tests} /></DashboardModalShell>
}
...
function ExamGoalProgress({ goals, attempts, tests }: { goals: { id: string; name: string; examId?: string; examName?: string }[]; attempts: Submission[]; tests: MockTest[] }) {
  ...
  if (!goals.length) return <Empty text="This user is not in a group with an exam goal." />
  return <section className="space-y-6"><div><h3 className="text-xl font-bold tracking-tight text-slate-900">Exam goal progress</h3><p className="mt-1 text-sm text-slate-500">Performance is calculated from this user’s submissions for each exam assigned through their groups.</p></div>{visibleGoals.items.map(goal => {
...
function DashboardModalShell({ label, name, activeTab, tabs, setTab, edit, close, children }: { label: string; name: string; activeTab: ModalTab; tabs: readonly (readonly [ModalTab, string])[]; setTab: (tab: ModalTab) => void; edit?: () => void; close: () => void; children: React.ReactNode }) {
  return <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 grid place-items-center bg-slate-950/50 p-4" onMouseDown={close}><div className="h-[90vh] w-full max-w-5xl overflow-auto rounded-2xl bg-white shadow-2xl" onMouseDown={event => event.stopPropagation()}><div className="flex items-start justify-between gap-3 border-b border-slate-200 p-6"><div><p className="text-xs font-bold tracking-widest text-indigo-600">{label}</p><h2 className="mt-1 text-2xl font-black">{name}</h2></div><div className="flex gap-2">{edit && <button type="button" onClick={edit} className="rounded-lg bg-indigo-600 px-3 py-2 text-sm font-bold text-white hover:bg-indigo-700">Edit group</button>}<button type="button" onClick={close} className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-bold">Close</button></div></div><div className="flex overflow-x-auto border-b border-slate-200 px-4">{tabs.map(([id, tabLabel]) => <button key={id} type="button" onClick={() => setTab(id)} className={`shrink-0 border-b-2 px-3 py-3 text-sm font-bold ${id === activeTab ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-slate-500'}`}>{tabLabel}</button>)}</div><div className="space-y-6 p-6">{children}</div></div></div>
}
