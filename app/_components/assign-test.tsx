...
export function AssignTest({ testId }: { testId: string }) {
  ...
  useEffect(() => {
    ...
    }).catch((reason) => { if (active) setMessage(`Could not load batches: ${reason.message}`) })
    return () => { active = false }
  }, [profile?.role, user])
  ...
  async function search(event: FormEvent<HTMLFormElement>) {
    ...
    } catch (reason) { setMessage(reason instanceof Error ? reason.message : 'Unable to search students.') }
  }
  ...
  async function assignGroup() {
    ...
    if (!user || !date || !group) { if (!group) setMessage('Choose a batch.'); return }
    if (group.members.length === 0) { setMessage('This batch has no members.'); return }
    setSaving('group')
    try {
