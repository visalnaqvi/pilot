...
function AnswerCard({ answer, canGrade, grade, setGrade }: {
  ...
  if (answer.kind === 'short_answer') {
    return <article className="rounded-xl border border-amber-200 bg-amber-50 p-5">
      <div className="flex items-center justify-between gap-4"><p className="text-sm font-bold">Question {answer.questionIndex + 1}</p><span className="rounded-full bg-amber-100 px-2 py-1 text-xs font-bold text-amber-800">{answer.awardedMarks != null ? `${answer.awardedMarks}/${answer.marks} marks` : `${answer.marks} marks · review`}</span></div>
  ...
  return <article className={`rounded-xl border p-5 ${!resolved ? 'border-slate-200 bg-slate-50' : answer.isCorrect ? 'border-emerald-200 bg-emerald-50' : 'border-rose-200 bg-rose-50'}`}>
    <div className="flex items-center justify-between gap-4"><p className="text-sm font-bold">Question {answer.questionIndex + 1}</p>{resolved && <span className={`rounded-full px-2 py-1 text-xs font-bold ${answer.isCorrect ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'}`}>{answer.isCorrect ? `Correct · ${answer.marks} mark${answer.marks === 1 ? '' : 's'}` : 'Incorrect'}</span>}</div>
    <p className="mt-3 font-semibold">{answer.prompt}</p>
